...
- **Gotcha**: never write the literal string `[skip ci]` (or `[ci skip]`, `[no ci]`, `[skip actions]`) in a PR title, PR body, or commit message you intend to merge — squash-merge concatenates the PR body into the merge commit, and GHA reads `[skip ci]` *anywhere* in the message as "do not trigger any workflow for any push pointing at this commit, ever." A tag pushed onto such a commit silently never fires Actions. If you need to reference the directive in prose, use a backtick-fenced or visually broken form (e.g. `skip-ci`). This bit us once between v1.0.1 and v1.0.2.
